...
def search():
    ...
    form = SearchForm()
    if form.validate_on_submit():
        url = generate_url(form.address_city_state_zip.data,
                           form.fuel_type.data, form.distance_radius.data)
...
def directions(origin, destination):
    ...
    url = f'{URL_WITH_KEY}&origin={origin}&destination={destination}&travelmode=driving'
    return redirect(url)
...
def generate_url(location, fuel_type, range_in_miles):
    ...
    """
    URL_WITH_KEY = f'https://developer.nrel.gov/api/alt-fuel-stations/v1/nearest.json?api_key={str(Config.NREL_API_KEY)}'
    search_query = f'{URL_WITH_KEY}&location={location}&fuel_type={fuel_type}&radius={range_in_miles}&limit=25&access=public'
    return search_query
